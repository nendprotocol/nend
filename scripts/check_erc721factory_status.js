...
async function main() {
    ...
    if (total == 0) {
      ...
      let tx = await factory.deploy("Sample PeriFi NFT", "SPN");
      await tx.wait();
      total = 1;
    }
    var address = await factory.addressOf(total - 1);
    console.log("last created contract address:", address);

    let PeriFiNFT = await ethers.getContractFactory("PeriFiNFT");
    let periFiNFT = await PeriFiNFT.attach(address);
