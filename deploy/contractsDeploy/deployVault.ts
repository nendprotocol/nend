...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (true) {
    try {
      ...
      const vaultName = 'Commission Pool';
      const vaultNumber = 1;
      const VaultComissionPoolDeployment = await deploy('VaultComissionPool', {
        from: deployer,
        contract: 'Vault',
  ...
  while (true) {
    try {
      ...
      const vaultName = 'Curation Reward Pool';
      const vaultNumber = 3;
      const VaultCurationRewardPoolDeployment = await deploy('VaultCurationRewardPool', {
        from: deployer,
        contract: 'Vault',
  ...
  while (true) {
    try {
      ...
      const vaultName = 'Insurance Vault';
      const vaultNumber = 4;
      const VaultInsuranceVaultDeployment = await deploy('VaultInsuranceVault', {
        from: deployer,
        contract: 'Vault',
  ...
  while (true) {
    try {
      ...
      const vaultName = 'Ecosystem Fund';
      const vaultNumber = 5;
      const VaultEcosystemFundDeployment = await deploy('VaultEcosystemFund', {
        from: deployer,
        contract: 'Vault',
