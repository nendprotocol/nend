...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    // const VaultEcosystemFundDeployment : DeploymentExt = await deployments.get('VaultEcosystemFund');
        ...
        while(true) {
            try {
              const LiquidityPoolStakingExecuteBurn = await execute('LiquidityPoolStaking',
                {from: deployer, log: true},
                'burn', NendDeployment.address, ethers.utils.parseEther("20000000")
              );	
              break;
            }catch(err) {
              console.log(err);
...
export default func;
module.exports.dependencies = ['NendDeployment', 'LiquidityPoolStakingDeployment'];
func.tags = [ 'LiquidityPoolStakingExecuteBurn' ];
